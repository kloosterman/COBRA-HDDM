...
for ipar = 1:5
  ...
  plotSpread(num2cell(plotdat,1), 'distributionMarkers', 'o', 'distributionColors', cols, 'categoryLabels', lab'); 
  title(sprintf('%s\nN=%d', behav(itime).ddm.pardimord{ipar}, length(data)))
  width = 0.7;
  
  for i=1:size(plotdat,2)
    line([i-width/2 i+width/2]', [nanmean(plotdat(:,i)) nanmean(plotdat(:,i))]',  'Color', 'k', 'Linewidth', 3)
  end
  %   end
end
saveas(f, sprintf('COBRA_SDTvsDDM_%s.png', time{itime}))
orient landscape
