...
# run model
jobs = v.map(run_basicmodel, range(15)) # 4 is the number of CPUs

models = jobs.get()

a = gelman_rubin(models)
b = pd.DataFrame.from_dict(a, orient='index')
b.to_csv('run_basicmodel_gelman_rubin_vals_drop_lowdprime.csv')
...
#%% export data
m.save('hddmmodel_run_basicmodel__drop_lowdprime') # save to file
